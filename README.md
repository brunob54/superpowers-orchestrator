...
[![Version](https://img.shields.io/badge/version-7.52.0-white?style=for-the-badge)](RELEASE-NOTES.md)
...
> [!NOTE]
> **Lineage & status:** this repository builds on two origins — the original [obra/superpowers](https://github.com/obra/superpowers) by Jesse Vincent and its optimized fork [REPOZY/superpowers-optimized](https://github.com/REPOZY/superpowers-optimized) (baseline v6.6.1). Full credit to both. The project was named *superpowers-optimized* through v6.15.1 and was renamed to **superpowers-orchestrator** at v7.0.0, to match its main feature: the autonomous orchestration pipeline. This fork's own additions (v6.7.0–v7.52.0) are **under testing and evaluation** — see [What this repo adds](#what-this-repo-adds).
>
> **How this fork is built:** every release of this fork (v6.7.0–v7.52.0) was designed, implemented, and reviewed end-to-end by Claude Fable 5 running in Claude Code — with the plugin itself driving its own development. (Batched implementation dispatches task subagents on smaller Claude models where the plugin's model-selection rules call for it; design, orchestration, and review stayed on Fable 5.) The fork bootstraps on its own releases: v6.7.0 was built under the parent fork plugin (baseline v6.6.1), and each release since was built with the fork's *previous* release installed. Every workflow described below was used, under real conditions, to build the version you're reading about.

## What this repo adds

Eighteen releases beyond the REPOZY v6.6.1 baseline. The six main additions — full guide with usage, details, and motivations in [docs/FORK-IMPROVEMENTS.md](docs/FORK-IMPROVEMENTS.md):
...
- **multi-doc-review (v6.9.0)** — N independent clean-context review rounds with rotating lenses on every spec and plan before its approval gate, with a sidecar audit log; since v7.4.0 each round can dispatch M (reviewers per lens) identical reviewers in parallel (`M=<m>`, 1–5, default 1) and consolidates their reports before triage; automatic at the gates, each of which asks you for N and M, or direct: `/multi-doc-review docs/superpowers-orchestrator/<date>-<slug>/specs/<slug>-design.md 3 M=2`. [Details](docs/FORK-IMPROVEMENTS.md#3-multi-doc-review--n-round-independent-document-review-v690)
...
The remaining releases — `multi-doc-review` rename (v6.11.0), plan-scoped SDD workspace (v6.12.0), fresh-session plan handoff (v6.13.0), cap-only batch boundary (v6.15.0), statusline bridge installer (v6.15.1), the rename to superpowers-orchestrator (v7.0.0), the spec gate's fresh-session orchestration route (v7.0.1), slug-stamped commit messages (v7.1.0), the per-topic artifact layout (v7.3.0), M reviewers per lens (v7.4.0), blocking controller dispatch (v7.5.0), tested harness claims in reviews (v7.6.0), plan contracts instead of literal bodies (v7.7.0), in-run rulings (v7.8.0), prompt-pointer dispatch in the code review loop (v7.9.0), prompt-pointer dispatch of the orchestrator's own controllers (v7.10.0), a user's plan amendment backing its marker (v7.11.0), review gates that ask for M (v7.12.0), the `<superpowers-defaults>` session block (v7.13.0), the Execution readiness pass at the plan review gate (v7.14.0), the stop report's answer-slot resume prompt (v7.15.0), the spec-deviation line of the plan review (v7.16.0), the plan-ruling count of the in-run resume cap (v7.17.0), the fix cycle of the decisions addendum's re-review (v7.18.0), the orchestrator's compaction recovery paragraph (v7.19.0), the `handoff` skill (v7.20.0), the whole-file hand-over rule (v7.21.0), the batch controller's filled worker templates (v7.22.0), the `pickup` skill (v7.23.0), the plan writer's repository-premise test (v7.24.0), behavioural suites that run in an empty work folder (v7.25.0), corrected statements about the context gate and about N = 0 (v7.26.0), smart-compress without its 300-second limit (v7.27.0), smart-compress that leaves compound commands uncompressed (v7.28.0), code reviewers that write their full report to a file (v7.29.0), and the compaction probe run with no person present, and a recovery guard that rejects a summary's claim of a done re-read and accepts a `sed -n` read (v7.30.0), and a session-start hook whose output stays under Claude Code's 10,000-character limit for hook output, so that the whole injection reaches the model (v7.31.0), and hooks that keep their files out of `git status` through git's local exclude file instead of editing `.gitignore` (v7.32.0), and ruling records in the shape a stop reads, with plan edits only under `amend plan` (v7.33.0), and a resume that finds the ruling commit of a ruling that is not the first of its return, and that restores only the amended clause (v7.34.0), and a revert that leaves a later amendment of the same clause in place, that cannot be run a second time, and that puts back the text it replaced when it has to stop (v7.35.0), and an audit note whose quote still finds its clause after the plan has changed and after a task checkbox has been ticked (v7.36.0), and a revert that reads the plan's own text, because every diff of the plan file now turns off the two helper programs a repository can configure (v7.37.0), and a code reviewer that reads the real code, because the review package and every diff a reviewer runs itself turn the same two helper programs off (v7.38.0), and an audit note whose quote is built under the same comparison the revert uses, with a marker lookup for an Exact-content block and a named later ruling when the quote finds no clause (v7.39.0), and a recorded answer that quotes the clause your decision leaves in force, read from the plan after the resume's own edits (v7.40.0), and a resume that stops when an earlier session left a plan edit uncommitted, with a code-review fix that never edits the plan file (v7.41.0), and a stop hook that no longer counts the throwaway edits of a subagent's own worktree, with test suites that stop when a check calls a command that does not exist (v7.42.0), a stop hook that keeps its save state per session, with a guard that also sees an undefined call inside a condition (v7.43.0), and a resume that stops on a code revert that an earlier session left unfinished, with a stop hook that reads `stop_hook_active` and a test that compares every version place (v7.44.0), one edit log per session with no cleanup after a refused revert (v7.45.0), a revert path list that holds both names of a renamed file, a check for ignored files before a revert and one statistics file per session (v7.46.0), one fixed check script that runs before every code revert (v7.47.0), a code revert that stays inside the files of the fix after a later rename (v7.48.0), a check script that reads the index bits and the sparse-checkout setting (v7.49.0), an orchestrator that no longer reverts a fix commit by itself (v7.50.0), and an orchestration skill that runs on Claude Code only (v7.51.0), and a `worklog` skill that keeps one tracking document per piece of multi-part work, named by the session-start hook (v7.52.0) — are covered in [RELEASE-NOTES.md](RELEASE-NOTES.md).
...
Automatically excluded from `git status` through the repository's local exclude file (`git rev-parse --git-path info/exclude`), which git never commits — it's a tooling artifact, not project code. No tracked file such as `.gitignore` is edited.
...
The remaining four are read by hook code directly and never reach a skill.
...
- **track-edits** (PostToolUse: Edit/Write) — Logs file changes for TDD reminders; on first write, excludes AI workspace artifacts (`project-map.md`, `session-log.md`, `state.md`, `known-issues.md`) from `git status` through the repository's local exclude file, never through `.gitignore`
- **track-session-stats** (PostToolUse: Skill) — Tracks skill invocations for progress visibility
- **stop-reminders** (Stop) — Surfaces TDD reminders, commit nudges, and session summary after each response turn. Switch off single reminders with `SUPERPOWERS_STOP_REMINDERS_OFF` (see **Environment variables** above)
