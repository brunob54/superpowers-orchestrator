...
Current branch and HEAD: !`git branch --show-current 2>/dev/null || echo none` at !`git rev-parse --short HEAD 2>/dev/null || echo none`
...
3. **Write the prompt** with exactly these sections, in plain English, no idioms,
   ...
   - **Header line.** The file's first line is exactly
     `Handoff: written=<YYYY-MM-DD>T<HH:MM> branch=<name|none> head=<sha7|none>`,
     with the date and time, branch and HEAD printed above (`none` for an empty
     value). The /pickup skill reads it to find the newest handoff and the commits
     made after it. When the next task has an end state that a commit may not show,
