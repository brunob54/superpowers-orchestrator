...
if [ -L "$HOME/.config/opencode/plugins/superpowers-orchestrator.js" ]; then
    echo "  [PASS] Plugin symlink exists"
else
    echo "  [FAIL] Plugin symlink not found at $HOME/.config/opencode/plugins/superpowers-orchestrator.js"
    exit 1
fi

# Verify symlink target exists
if [ -f "$(readlink -f "$HOME/.config/opencode/plugins/superpowers-orchestrator.js")" ]; then
