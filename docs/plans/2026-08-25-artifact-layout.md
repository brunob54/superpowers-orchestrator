...
_Note: the shipped suite in tests/claude-code/test-multi-code-review.sh is authoritative. Review fixes on this branch changed the Case 2 checks (exact-subject (p3a)/(p3b), an ungated (p5-control), a (p5-control2) range check, and the (h3) effective-HEAD check on the completion marker); the fenced blocks below were re-synchronized afterwards and are verbatim copies of the shipped file._
...
(b) Immediately after the `trap "cleanup_test_project '$TEST_PROJECT'" EXIT` line, define the two helpers the Case 2 guard and the summary use: `project_dirt` (uncommitted changes in the test project, transcripts excluded) and `finish` (prints the summary, disarms the EXIT trap on failure so the project and its transcripts survive, and exits with the verdict's code):
...
(c) Insert the second case between the existing direct-mode assertion block (the `if [ -z "$LOG" ] …` block, closed by its `fi`) and the file's final summary call. Do **not** append at the end of the file: `finish` prints the verdict and calls `exit`, so every `FAILURES=$((FAILURES+1))` placed after it would have no effect and the Case-2 assertions would pass vacuously. The case opens with a guard: the script runs under `set -euo pipefail`, so a `git checkout -b` that fails — a dirty tree left by Case 1, or a branch of the same name that already exists — would abort the script before the summary prints and let the EXIT trap delete the transcripts. The guard records each condition as a numbered `FAIL(setup)` and goes straight to the summary; when it passes, Case 2 starts from `$BASE_SHA` on a fresh branch. The fix-reports assertions (p6, p7) and the second (p5) needle are gated on the same `FIX_COMMITS` condition as the (p5-control) check: without a fix commit there is no fix report to write.
...
(d) Replace the file's final summary block (`if [ "$FAILURES" -eq 0 ]; then … fi`, which printed the verdict, disarmed the trap and exited) with the single call:
