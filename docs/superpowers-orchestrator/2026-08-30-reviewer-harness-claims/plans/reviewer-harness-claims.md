...
> **Amendment 2026-08-30 (review v4–v6 items, user decision):** three further rulings, applied identically in `skills/multi-code-review/reviewer-prompt.md` and `skills/multi-doc-review/reviewer-prompt.md` (the two `### Harness claims` sub-sections stay byte-identical) and, for ruling (3), also in `skills/multi-code-review/SKILL.md` and `skills/multi-doc-review/SKILL.md`. (1) [v4 I3] + [v5 I2] — the model-probe example is no longer contaminated: the token is a random string placed only in the dispatch's `description`; the prompt given to the probe subagent never contains the token — it asks the subagent to report every string in its context that looks like a dispatch label or an unexplained token, or to report that it sees none; the token reported absent is the observation. (2) [v4 I4] — the tie-break "can its truth be read from the repository?" is replaced by a definition: a harness property is a claim about the agent runtime that is running this review (the tool that dispatched the reviewer and its hooks, tools, context delivery and environment); its truth can only be observed by running that runtime; a claim about a library, a language runtime, the operating system, or a remote service is not a harness property — it is an ordinary claim, checked against its source or documentation and referenced like any other finding. (3) [v6 I1] — a way back for a mis-tagged premise: both templates gain the sentence "Do not tag a claim whose truth can be read from the repository or from a source you can cite; the `harness:` field is only for runtime properties", and the controller triage of harness claims in both SKILL.md files gains, before the item that handles `harness: untested` findings, the strip rule: a `harness:` field on a premise that can be read from the repository or from a citable source is dropped — triage the finding as an ordinary finding under the existing reference requirement and append `(harness field dropped: repository-readable)` to its disposition line.
...
> **Amendment 2026-08-30 (review v2/v3 items, user decision):** four further rulings, applied identically in `skills/multi-code-review/SKILL.md` and `skills/multi-doc-review/SKILL.md` except where noted. (1) [v2 I2] + [v3 I2] — the rejection carries a reason: `rejected: harness probe not runnable here — <probe> — (<reason>)`, where `<probe>` is the reviewer's probe text copied verbatim (the `first:` text of a `not settled by one probe` tag, the probe the controller named for an untagged premise, or the literal `none` when no probe exists) and `<reason>` is one of `not settled by one probe`, `probe subagent did not report`, `no probe named`, `tool missing`, `would break a constraint`, `ambiguous observation`; the completion report's item becomes `- [<id>] <probe> — (<reason>) (round <i>)`, with `(addendum)` in place of `(round <i>)` for a rejection made in a post-loop addendum, and the reason clause copied from the rejection line. (2) [v3 I1] — the poll of the dispatch rule is concrete: `test -s <path>` as its own tool call, repeated as separate tool calls (never a shell loop, never `sleep`, which some harnesses refuse in the foreground), at most 20 attempts, spread over the controller's own remaining work; a final message that did arrive always wins over the file; the path is best-effort and a slower probe subagent is reported as `probe subagent did not report`. (3) [v2/v3 I3] — an observation that neither matches nor contradicts the predicted result (including one naming neither the predicted result nor its negation) is not an observation: the finding is treated as `harness: untested` with the same probe; in `multi-code-review` only, the guard also re-runs, before logging it `user-decision`, the probe of a finding whose `tested` tag came from the reviewer (a probe the controller ran itself in the same round is never repeated; a context-read probe is re-run from a throwaway subagent's position, never from the controller's own context), and takes the "not runnable here" branch when it cannot. (4) [v2/v3 I3] — the consolidation rules of both files gain: a consolidated finding is `harness: tested` only when every source that carries a `harness:` field is `tested`; otherwise it is `harness: untested` with the probe of the lowest-numbered source tagged `untested`.
