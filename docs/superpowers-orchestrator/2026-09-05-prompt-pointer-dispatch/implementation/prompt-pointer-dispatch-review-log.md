...
### Post-loop addendum — 2026-09-06 — decisions
Effective HEAD had moved (the `_Completed` line above was written without its HEAD sha; the effective HEAD at marker time was 93b9456, the round 4 verification 2 fix; effective HEAD 8d906dc at answer time: spec Amendment 2 and plan amendments by rulings 5–7); the marker above is left unchanged, the verification re-review of the accepted fixes is skipped, and Invocation 3 below reviews them. The two `(orchestrator)` `fix it` answers are applied: the round 4 [I1] clause sits in a fenced block of Task 3 Step 3 (reference text under the plan's Body authority note; ruling 5), and the round 4 verification 3 [I1] item carries `clause: none` (ruling 7).
- [I1] decided (orchestrator): fix it: (this answers the round 4 user-decision item on lens-file reuse) the verification-cycle row of the file-name table in skills/multi-code-review/SKILL.md, and the sentence after the table, say that a verification cycle reuses its round's lens file when the current prompt directory holds it and otherwise writes it first — a resumed or post-loop-addendum controller starts from a fresh directory (ruling 3) that holds no earlier file; the same rule for a fix re-dispatch's findings file (round 4 item)
- [I1] decided (orchestrator): fix it: (this answers the round 4 verification 3 unresolved item on the restore rule) in fix-prompt.md step 2 and the SKILL.md fix-failure bullet, the restore after a failed fix attempt uses git checkout -- <path> only for files git tracks; a file the attempt created and git does not track is removed by explicit path (rm -- <path>), never git clean, so the re-dispatch starts on a clean tree (round 4 verification 3 item)
- [I3] decided (user): amend plan: when hooks/safety/protect-secrets.js refuses a value-file Write, each line the hook's message names is replaced by its file:line plus the fixed text "secret-bearing finding, value withheld" and the Write is retried once; a second refusal is fatal (Global Constraints amended by ruling 6; Task 3 Contract amended; spec Amendment 2); fix it: skills/multi-code-review/SKILL.md's Error Handling section and the Critical/Important fix bullet state that rule, the withheld finding keeps its id and severity in the findings file so the fix subagent still removes the secret at that location, every other value-file failure stays fatal; tests/reviewer-templates/run-tests.sh section 10 asserts `value withheld` in the Error Handling range (round 4 item)
- [I1] fixed — the file-name table's four reuse cells, the sentence after the table and the fix-failure bullet now say a value file (a verification cycle's lens file, a re-dispatch's findings file) is reused when the current prompt directory holds it and written first when it does not; a resumed or post-loop-addendum controller starts from a fresh directory → 9de4fbe (round 4 item)
- [I1] fixed — fix-prompt.md step 2 and the SKILL.md fix-failure bullet restore tracked files with `git checkout -- <path>` and remove a file the failed attempt created by explicit path (`rm -- <path>`), never `git clean` → 9de4fbe (round 4 verification 3 item)
...
## Round 6 — Adversarial red-team — fable
**Reviewers:** M=2, usable 2/2
**Reviewer verdicts:** r1: 0 Critical, 3 Important, 3 Minor | r2: 0 Critical, 2 Important, 3 Minor
**Sources mapped:** 11/11
**Reviewer verdict:** 0 Critical, 4 Important, 6 Minor
**Converged:** no
### Dispositions
- [I1] rejected: duplicate of round 5 [I2], an open user-decision item (hooks/safety/protect-secrets.js names a credential kind and never a line, so the withholding rule cannot be executed as written; the same wording stands in the amended Global Constraints and in the spec) ← 2/2: r1:I1, r2:I1
- [I2] user-decision — the u = 0 rule makes a round whose reviewers both failed for a reason unrelated to the prompt file (rate limit, tool error, killed subagent) return 'BLOCKED: no reviewer of round <i> could use its prompt file', a stop the user chose in Invocation 1's addendum ([I1], ruling 4 follow-up, spec Amendment 1) and that Invocation 2 round 4 verification 2 [I4] held as decided wording; the suggested carve-out (continue as inconclusive when the final messages show the file was read) contradicts the amended constraint (plan-mandated) — at skills/multi-code-review/SKILL.md:557 — clause: Global Constraints "Every failure of the mechanism (`mktemp -d` or `cygpath`, the script, `test -s`, Node missing, a value-file write that fails or that `hooks/safety/protect-secre" ← 1/2: r1:I2
...
- [I4] user-decision — a controller's own fill-command slip (exit 1, 3 or 4, or exit 5 naming an @file it never wrote, for example the round-1 CARRIED_BLOCK form copied into round 2) is a mechanism failure by the letter and returns BLOCKED although nothing was written; the suggested carve-out (correct the command and run it once more, a second non-zero exit fatal) contradicts 'every failure of the mechanism (the script included) is fatal' (plan-mandated) — at skills/multi-code-review/SKILL.md:487 — clause: Global Constraints "Every failure of the mechanism (`mktemp -d` or `cygpath`, the script, `test -s`, Node missing, a value-file write that fails or that `hooks/safety/protect-secre" ← 1/2: r2:I2
- [M1] carried — the fill-prompt.js exit-5 message 'file already exists' for an existing --out with different content reads like the sanctioned identical repeat (which exits 0 silently); saying 'with different content' would tell them apart ← 1/2: r1:M1
- [M2] rejected: duplicate of Invocation 2 round 4 [M3] (carried; an empty or newline-only findings file removes the whole-line placeholder silently and is unreachable by the procedure, which dispatches a fix only for a non-empty Critical/Important list) ← 1/2: r1:M2
- [M3] carried — the prompt directory is never removed and accumulates finding and failure text in the temp area; the Global Constraints say the directory is never cleaned up by the skill and the plan lists cleanup as a non-goal ← 1/2: r1:M3
...
### Post-loop addendum — 2026-09-06 — decisions
Effective HEAD had moved past the marker (9aac7f6 → d91e0c8: spec Amendment 3 and the ruling 8 plan amendment landed after the stop), so the marker above stays unchanged, the verification re-review of the accepted fixes is skipped, and Invocation 4 below reviews them.
- [I2] decided (user): (this answers the round 5 item on the withhold mechanics) amend plan: when hooks/safety/protect-secrets.js refuses a value-file Write, the controller runs `node hooks/safety/protect-secrets.js` itself on each line of that file with a synthetic Write payload in the form the hook reads, replaces every line the hook refuses by its file:line plus the fixed text "secret-bearing finding, value withheld" (the withheld finding keeps its id and severity), and retries the Write once; a second refusal is fatal (Global Constraints amended by ruling 8; Task 3 Contract; spec Amendment 3); fix it: skills/multi-code-review/SKILL.md's Error Handling section and the Critical/Important fix bullet state the per-line probe with the hook's actual invocation and payload form, read from hooks/safety/protect-secrets.js (round 5 item)
- [I2] decided (user): (this answers the round 6 item on the u = 0 rule) amend plan: a round with no usable report after the pointer dispatch and the one identical retry is fatal only when at least one reviewer's final message shows it could not read or did not follow its prompt file; a round whose final messages all show an environment death (usage limit, tool error, no message at all) is not a mechanism failure and is logged inconclusive as today, the loop continuing (Global Constraints amended by ruling 8; Task 3 Contract; spec Amendment 3); fix it: skills/multi-code-review/SKILL.md step 3 and the Error Handling section state that distinction and the BLOCKED text for the fatal case (round 6 item)
- [I4] decided (user): (this answers the round 6 item on fill-command slips) amend plan: when fill-prompt.js exits 1, 3 or 4, or exits 5 naming an @file the controller never wrote, the controller corrects its own command once and runs it again; a second non-zero exit is fatal; exit 2, and exit 5 on a file the controller did write, are fatal at once (Global Constraints amended by ruling 8; Task 3 Contract; spec Amendment 3); fix it: skills/multi-code-review/SKILL.md step 2, the fix bullet and the Error Handling section state that rule; tests/reviewer-templates/run-tests.sh section 10 assertions updated only where the exact wording they pin changed (round 6 item)
- [I2] fixed — the Critical/Important fix bullet and the Error Handling value-file row now say the hook's refusal names only a credential kind, never a line, so the controller runs `node hooks/safety/protect-secrets.js` once per line of the refused file with the stdin JSON object the hook reads (tool_name Write, tool_input.file_path and tool_input.content), treats a printed "permissionDecision":"deny" as the refusal, replaces each refused line by its file:line plus the fixed text, keeps the finding's id and severity, retries the Write once, and treats a second refusal as fatal → b52a9f2 (round 5 item)
- [I2] fixed — step 3 and two Error Handling u = 0 rows split the case: BLOCKED with the existing text only when at least one final message shows a reviewer that could not read or did not follow its prompt file; a round whose final messages all show an environment death (usage limit, tool error, no message) stays inconclusive and the loop continues → b52a9f2 (round 6 item)
- [I4] fixed — step 2 (the CARRIED_BLOCK exit-5 sentence and the test -s sub-step), the fix bullet and two new Error Handling rows state that exit 1, 3 or 4, and exit 5 on an @file never written, are the controller's own slip, corrected once and re-run, a second non-zero exit fatal, exit 2 and exit 5 on a written file fatal at once; no section-10 assertion pinned wording that changed, so tests/reviewer-templates/run-tests.sh is unchanged (60/60 pass) → b52a9f2 (round 6 item)

_Invocation 4 — 2026-09-06 — N=2 M=2 — BASE..HEAD b9b9ffb..b52a9f2 — branch feature/prompt-pointer-dispatch — gate: orchestration_

## Round 7 — Correctness & spec alignment — fable
**Reviewers:** M=2, usable 2/2
**Reviewer verdicts:** r1: 0 Critical, 3 Important, 3 Minor | r2: 0 Critical, 4 Important, 5 Minor
**Sources mapped:** 15/15
**Reviewer verdict:** 0 Critical, 4 Important, 8 Minor
**Converged:** no
### Dispositions
- [I1] user-decision — the per-line secrets probe is written `node hooks/safety/protect-secrets.js`, a path relative to the repository root, while the hook lives in the plugin (hooks/hooks.json registers it through CLAUDE_PLUGIN_ROOT): in any project other than this plugin's own repository Node exits 'Cannot find module', prints nothing, no line reads as refused, the unchanged Write is refused a second time and the run ends BLOCKED, so the sanctioned exception can never run outside this repository; the suggested fix (`node '<skill-dir>/../../hooks/safety/protect-secrets.js'`, a non-object output treated as a failure) changes the command the amended Global Constraints spell, which is the user's decided wording of ruling 8 (plan-mandated) — at skills/multi-code-review/SKILL.md:772 — clause: Global Constraints "Three bounded exceptions: (1) when `hooks/safety/protect-secrets.js` refuses a value-file Write, the controller runs `node hooks/safety/protect-secrets.js` itse" ← 2/2: r1:I1, r2:I2
- [I2] fixed — the synthetic payload put the finding line raw inside a JSON string, so a line holding a quote or a backslash was malformed JSON, the hook printed `{}` and the line read as allowed; the fix bullet and the Error Handling value-file row now build the payload by program with the line as an escaped JSON string literal, fed through a quoted heredoc, and only a JSON object carrying no deny decision allows a line, every other result (deny, a denied probe command, a non-zero exit, a non-object output) withholding it → 7aec7e0 ← 2/2: r1:I2, r2:I3
- [I3] fixed — the Procedure's 'Before round 1' fatal list and step 6 still stated the pre-Amendment-3 rule (every fill-script non-zero exit fatal, every u = 0 round fatal, an inconclusive round never continued past); the list now names the three bounded exceptions and qualifies the two entries, the unconditional 'In every case' clause is gone, and step 6 says an inconclusive round is never clean and breaks the streak while step 3 decides whether the loop continues → 7aec7e0 ← 2/2: r1:I3, r2:I1
- [I4] fixed — the restore after a failed fix attempt ran `git checkout -- <path>` on every tracked file the attempt changed, which in a direct-mode run started over consented pre-existing changes discarded the user's uncommitted work; the restore now applies only to files clean at loop start, the controller names the paths already modified at loop start in the findings value file, the fix subagent leaves such a file as the attempt left it and names it in its failure report, and the re-dispatch failure text says so (SKILL.md fix-failure bullet, fix-prompt.md step 2 and Findings section) → 7aec7e0 ← 1/2: r2:I4
- [M1] fixed — the probe command's standard-input delivery was unstated and could be denied by the plugin's Bash hooks; the line is now fed through a quoted heredoc, never as an echo or printf argument, and a probe command a hook denies counts as a refused line → 7aec7e0 ← 1/2: r1:M1
- [M2] fixed — fix-prompt.md never explained the withheld form to the fix subagent; its Findings section now says a line whose description is the fixed withheld text reports a hardcoded credential at that location, to be removed and loaded from the environment → 7aec7e0 ← 1/2: r1:M2
- [M3] rejected: the loop never edits plan text — the plan header's Architecture paragraph and its Git Bash assumption still describe the inline fallback that the amended Global Constraints removed (reference text under the body-authority note; the orchestrator owns the plan file) ← 1/2: r1:M3
- [M4] carried — fill-prompt.js dropTrailingBlank drops trailing empty body lines, but an @file value ending in several newlines is one element holding newlines, so the output can end with more than one newline ← 1/2: r2:M1
- [M5] fixed — fill-prompt.js said 'file already exists' on exit 5 when the existing --out path could not be read (a directory, an unreadable file); it now reports the read error's message, and the 'already exists' text only when the read succeeded with different content → 7aec7e0 ← 1/2: r2:M2
- [M6] fixed — step 2 sub-step 3 and the fix bullet ran `test -s` before reading the script's exit code; the exit-code paragraph with the one corrected rerun now precedes the `test -s` sentence in both places → 7aec7e0 ← 1/2: r2:M3
- [M7] fixed — the skill never said the prompt directory is outside the checkout; `mktemp -d` is now run with no argument, never a path inside the repository, and a printed path under the root anchor is a `mktemp -d` failure → 7aec7e0 ← 1/2: r2:M4
- [M8] fixed — Error Handling said protect-secrets.js scans the whole Bash command string, a heredoc body included; it now says the hook applies only its file-access patterns on Bash and scans hardcoded-secret content on Write and Edit alone → 7aec7e0 ← 1/2: r2:M5
- Carried findings from `.superpowers/sdd/progress.md` (18 `Minor:` lines, the same ledger as Invocations 1 to 3), decided from the two reviewers' recommendations (both recommended ship-as-is on every item):
- [M9] carried — fill-prompt.js writeAtomic: a body of only whole-line placeholders with empty values writes a 1-byte file and exits 0 (unreachable with the real templates)
- [M10] carried — fill-prompt.js writeAtomic catch unlinked a temporary path this process may not have created (already fixed in Invocation 1, 2ade665)
- [M11] carried — fill-prompt.js end-of-line detection is whole-file; the comment does not say so
- [M12] carried — tests/fill-prompt section 5: the 'missing @file exits 5' case lacked the assert_absent check (already fixed in Invocation 1, 2ade665)
...
- [M22] carried — tests/reviewer-templates section 10: the Procedure range also spans Lens Rotation (plan-mandated end marker)
- [M23] carried — SKILL.md Critical/Important bullet: the fix-dispatch description and 'the fix-subagent model of Parameters' are pinned by no section-10 needle
- [M24] carried — SKILL.md step 2 sub-step 2: 'every value except LENS_INSTRUCTIONS and a non-empty CARRIED_BLOCK is inline' (already reworded to 'inline by default' in Invocation 2, 15efd82)
- [M25] carried — Task 3 security review residual M3 (verification-cycle reviewer prompt beside its round's fix prompt in one flat directory), decided as [M19] of Invocation 1 (ruling 2), [M25] of Invocation 2 and [M21] of Invocation 3; both reviewers now recommend ship-as-is (the spec's error-handling table accepts this exposure)
- [M26] carried — Task 4 produced no commit: CLAUDE.md is gitignored (.gitignore:7), so the branch history holds no record of the Testing-block edit beyond the plan tick and ruling 1 (traceability note only)

## Round 8 — Adversarial red-team — fable
**Reviewers:** M=2, usable 2/2
**Reviewer verdicts:** r1: 1 Critical, 3 Important, 3 Minor | r2: 0 Critical, 1 Important, 3 Minor
**Sources mapped:** 11/11
**Reviewer verdict:** 1 Critical, 3 Important, 5 Minor
**Converged:** no
### Dispositions
- [C1] fixed — the probe outcome rule read every non-zero exit as a refused line, so a hook that cannot start (module not found in any repository other than this plugin's own, a crash) withheld every line of the file, the retried Write passed with no real description left, and the fix subagent edited code at every location as if it held a credential with no BLOCKED; the fix bullet and the Error Handling value-file row now split the probe into four outcomes: a JSON object with no deny decision allows the line, a deny decision withholds it, a probe command denied by a Bash hook withholds it, and a hook that printed no JSON object is a mechanism failure returning BLOCKED naming the probe, never a withheld line; the bare command path itself is round 7 [I1], an open user-decision item this fix does not touch → f1fde9e ← 2/2: r1:C1, r2:I1
- [I1] fixed — a withheld line was described to the fix subagent as a hardcoded credential to remove, although withholding also covers a line whose probe command a Bash hook denied for the finding's own wording (a finding that quotes a forced git clean, a command printing a secrets file, or an environment dump), so the fix subagent made a wrong edit at a location with no credential and lost the real defect; fix-prompt.md now makes the instruction conditional (inspect the location, remove a credential when one is there, otherwise leave the finding unfixed and report its id back as withheld) and SKILL.md states the same with the controller disposition `unresolved: withheld finding, no credential at the location` (blocking) → f1fde9e ← 2/2: r1:I2, r2:M1
- [I2] user-decision — the u = 0 decision has two cases only, a reviewer that could not read or did not follow its prompt file (fatal) and an environment death (inconclusive); a reviewer that read the file, reviewed the diff and returned a report unusable on format alone (a preamble line before the marker, no Verdict block) fits neither, and a controller applying 'did not follow it' literally ends the run on a formatting slip that the identical retry repeats under M = 1, where the pre-branch rule logged inconclusive and moved on; the suggested narrowing (fatal only when the final message shows no sign of the prompt file's content) narrows the amended Global Constraints clause, the user's decided wording of ruling 9 (plan-mandated) — at skills/multi-code-review/SKILL.md:586 — clause: Global Constraints "Every failure of the mechanism (`mktemp -d` or `cygpath`, the script, `test -s`, Node missing, a value-file write that fails or that `hooks/safety/protect-secre" ← 1/2: r1:I1
- [I3] rejected: harness probe not runnable here — in a default-permission-mode session, dispatch one throwaway `general-purpose` subagent with the three-sentence pointer to a one-line file under a fresh `mktemp -d` and observe whether its Read succeeds without a permission prompt — (ambiguous observation) — a prompt directory outside the checkout may make every reviewer's Read of its prompt file raise a permission prompt in a default-mode session, so round 1 would end BLOCKED on every default-mode machine; this session runs in bypass mode, so no observation here can match or contradict the claim ← 1/2: r1:I3
- [M1] rejected: duplicate of Invocation 3 round 6 [M5] (carried; the last-output-line checks on both real templates derive the expected line with the script's own closing-fence rule and pass vacuously on the untagged column-0 fence residue that the fill-prompt.js comment documents) ← 1/2: r1:M1
- [M2] fixed — fix-prompt.md's data-not-instructions rule covered finding text only, while the text under '## Previous attempt failed' quotes the branch's test output; the rule now covers both → f1fde9e ← 1/2: r1:M2
- [M3] fixed — the withheld-line replacement stated no line form and a finding without a file:line reference had nothing to put there; both places now prescribe the full line (id, severity, the file:line or the words 'no location', the fixed text); this also resolves Invocation 3 round 5 [M3], carried → f1fde9e ← 1/2: r1:M3
- [M4] fixed — an exit 5 for a cause other than an @file never written or a file the controller did write (an --out directory missing or unwritable, an existing --out with different content, a read error on it) fell in neither clause of step 2 sub-step 3 and Error Handling; both now say only an @file never written is corrected once and every other exit-5 cause, named from the script's source, is fatal at once → f1fde9e ← 1/2: r2:M2
- [M5] fixed — a refused line of round-<i>-failure.txt has no file:line and fix-prompt.md read any withheld line as a credential finding; the failure-file rule now replaces a refused line by the fixed withheld text alone and fix-prompt.md says a line reading only that text under '## Previous attempt failed' is omitted text, not a finding → f1fde9e ← 1/2: r2:M3

## Round 8 verification 1 — Adversarial red-team — fable
**Reviewers:** M=2, usable 2/2
**Reviewer verdicts:** r1: 0 Critical, 2 Important, 3 Minor | r2: 0 Critical, 1 Important, 4 Minor
**Sources mapped:** 10/10
**Reviewer verdict:** 0 Critical, 2 Important, 5 Minor
### Dispositions
- [I1] rejected: duplicate of round 7 [I1], an open user-decision item (the per-line secrets probe's bare path `node hooks/safety/protect-secrets.js` exists only in this plugin's own checkout; in any other repository the hook cannot start and, after the round 8 [C1] fix, the run ends BLOCKED naming the probe on every resume) ← 2/2: r1:I1, r2:I1
- [I2] fixed — the pre-re-dispatch restore used `git checkout -- <path>`, which restores from the index, so a file the failed attempt had already staged before its commit died kept the staged edit and the re-dispatch started on top of it; the SKILL.md fix-failure bullet and fix-prompt.md step 2 now unstage and restore a tracked file that was clean at loop start with `git checkout HEAD -- <path>` → 665c18c ← 1/2: r1:I2
- [M1] fixed — the u = 0 cases overlapped for a reviewer whose final message reports a failed Read of the prompt file (a pointer failure, fatal, or 'a tool error', environment death); step 3 and both Error Handling rows now say a tool error on the Read of the prompt file itself is the fatal pointer case and any other tool error is an environment death → 665c18c ← 2/2: r1:M2, r2:M1
- [M2] fixed — the pre-re-dispatch text ran the status check before the restore and gave no disposition for a path the check still shows; the steps are now ordered (restore, then check) and a path still shown is disposed of by its loop-start state: a tracked path that was clean is unstaged and restored by explicit path, an untracked path that did not exist is removed by explicit path, each named in the re-dispatch failure text → 665c18c ← 2/2: r1:M3, r2:M3
- [M3] carried — a fill that exits 0 with a wrong value has no sanctioned recovery: the corrected re-fill exits 5 on an existing file with different content, fatal at once, so the controller either returns BLOCKED or dispatches the wrong prompt; a suffixed second file or an explicit dispatch-as-is rule touches the once-written rule of the Global Constraints ← 1/2: r1:M1
- [M4] fixed — fix-prompt.md told the fix subagent that a '## Previous attempt failed' section below holds the failed attempt's output, although on a first dispatch that section is absent; the sentence is now conditional and says the section is absent on a first dispatch → 665c18c ← 1/2: r2:M2
- [M5] rejected: harness probe — observed: one Write tool call to a file under a fresh mktemp -d path completed with no permission prompt in the bypass-permissions mode this pipeline runs under — a permission prompt on the Write side of the value files would stop an autonomous run and the text gives it no disposition ← 1/2: r2:M4
