...
_Completed — 2026-09-12 — cap reached — HEAD 9f4ce7b93a28cff7f36c4fcbc43619574669f1a0_
Secrets found: none
...
### Decisions addendum (2) — 2026-09-12

Answers supplied by the orchestrator on a later dispatch, for the open item of invocation 2's `## Round 8 verification 4` entry.

- [I1] decided (orchestrator): fix it: (this answers the `## Round 8 verification 4` item on section 17 of tests/reviewer-templates/run-tests.sh, NOT the `N_code=0` item of tests/review-gates/run-tests.sh that ruling 5 already decided and that is already fixed) add one folded needle to section 17 of tests/reviewer-templates/run-tests.sh pinning the seventh Must-convey clause of Task 8's Contract — the earlier-release case of the completeness rule in Deviation 2 of skills/orchestrating-development/doc-review-loop-prompt.md, 'by whichever of its clauses applied, the clause for an entry written by an earlier release included, which owes no post-sequence' — so that deleting that clause from the template turns the suite red. Use the folded-needle helper of that suite, never a whole-line assertion, for the same reason as rulings 1 to 3 and 5. Verify with a negative check: delete the clause, confirm exactly that assertion fails, restore, confirm the suite is green again.
- [I1] fixed — one `assert_folded_contains` needle added at the end of section 17 of tests/reviewer-templates/run-tests.sh, pinning the earlier-release clause of Deviation 2 of skills/orchestrating-development/doc-review-loop-prompt.md; no whole-line assertion was used, so Global Constraint 1 Move 1's no-reflow list is unchanged. Negative check: with that clause deleted from the template the suite reported 192 passed, 1 failed, the one failure being exactly the new assertion; with the template restored (`git checkout --`) the suite is green at 193 passed, 0 failed, and the template file ends unchanged → f64f4ac

## Round 8 verification 5 — Test & coverage quality — opus
**Reviewers:** M=3, usable 3/3
**Reviewer verdicts:** r1: 0 Critical, 2 Important, 3 Minor | r2: 0 Critical, 1 Important, 4 Minor | r3: 0 Critical, 1 Important, 6 Minor
**Sources mapped:** 17/17
**Reviewer verdict:** 0 Critical, 2 Important, 11 Minor
### Dispositions
- [I1] unresolved: verification cap — the lens cell's clause-removal safety text is only half pinned; the cell clause 'a number missing from the list was removed on purpose for this plan; never reconstruct it', the no-renumbering paragraph and the sentence 'Write that block as the last lines of your report' carry no needle, so deleting any of them leaves every suite green — at tests/reviewer-templates/run-tests.sh:404 — clause: none ← 2/3: r1:I1, r2:I1
- [I2] unresolved: verification cap — section 14's needle loop asserts against the whole file instead of the '### Triage of a readiness finding' subsection and several of its needles also match outside it, so the 'fixed text vs fixed text' oscillation-guard row and Step 0's verify paragraph can each be deleted with every section 14 needle still green (one reviewer confirmed both by mutation on a temporary copy) — at tests/reviewer-templates/run-tests.sh:481 — clause: none ← 2/3: r1:I2, r3:I1
- [M1] carried — the 1080-line budget stands one line above the measured file (1079) while Global Constraint 1 still justifies the figure by a 34-line margin, so the next one-line wording fix turns the suite red ← 2/3: r1:M1, r2:M2
- [M2] carried — the rules that decide a readiness disposition carry no needle: Step 0's folded side-matching comparison, the check (5) all-sites rule, the empty Global Constraints block case and the third-open-pass rule ← 2/3: r1:M3, r2:M1
- [M3] carried — Task 2's Contract requires the new N = 0 sentence to follow the existing one while the suite asserts each string separately against the whole folded file, so the adjacency is unverified ← 1/3: r1:M2
- [M4] carried — Task 9's only verification of the three user-facing documents is a `grep -c` that passes on any single mention, so the release's user-facing wording is effectively unchecked ← 1/3: r2:M3
- [M5] carried — section 13's needles are asserted against the whole file although the suite already resolves the '### Readiness sequences' span, so a rule moved into another subsection keeps the suite green ← 1/3: r2:M4
- [M6] carried — only the second half of the lens cell's output-format mitigation is pinned; the sentence 'Write that block as the last lines of your report' carries no needle of its own ← 1/3: r3:M1
- [M7] carried — assertion descriptions in sections 13 and 14 name a section that does not hold the string they match, sending a maintainer to the wrong part of a 1079-line file ← 1/3: r3:M2
- [M8] carried — the needle `as a round is` is a generic 13-character fragment, so almost any reword of the clause it stands for still passes ← 1/3: r3:M3
- [M9] carried — Tasks 6 and 7 name section 15 of tests/review-gates/run-tests.sh as their verification while the assertions were implemented as section 16 ← 1/3: r3:M4
- [M10] carried — the Phase 0 sentence `N_plan=0 still dispatches the Phase 2 controller` is a Must-convey item of Task 7's Contract and carries no assertion ← 1/3: r3:M5
- [M11] carried — the size-budget check has no committed over-budget fixture, so the negative control Task 5's verification names exists only as a manual step ← 1/3: r3:M6
...
### Dispositions
- [I1] unresolved: verification cap — the two clause-removal anchor needles at tests/reviewer-templates/run-tests.sh:397 and :398 pin their anchors as folded substrings, so they stay green on a cell where the `:397` anchor begins no paragraph (the `:398` instruction says “the sentence beginning”, which is accurate); in skills/multi-doc-review/SKILL.md the Execution readiness cell holds no blank line and the string `For check (5) report ONE finding` occurs mid-sentence, so the removal instruction that names it as a paragraph has no locatable target — at tests/reviewer-templates/run-tests.sh:397 — clause: none
...
### Decisions addendum (4) — 2026-09-12

Answer supplied by the orchestrator on a later dispatch, for the one open item of invocation 2's `## Round 8 verification 6` entry.

- [I1] decided (orchestrator): fix it: (this answers the `## Round 8 verification 6` item at tests/reviewer-templates/run-tests.sh:397 on the two clause-removal anchor needles, NOT any earlier `[I1]` of this invocation, all of which are already decided and fixed) reword the clause-removal instruction in skills/multi-doc-review/SKILL.md — the Execution readiness lens-cell table row and the prose below it — so that it names its target as a bounded run of sentences with an explicit start and an explicit end, from `For check (5) report ONE finding` up to, but not including, the sentence beginning `Coverage, ambiguity, feasibility`, instead of “the paragraph beginning”; update the verbatim row needle at tests/reviewer-templates/run-tests.sh:437 in the same commit so that it matches the new wording; and add one `assert_folded_contains` needle pinning the reworded bounded-span wording, so that the fix is itself pinned. Add NO assertion that requires the anchor to begin a paragraph or a line, add no blank line to the cell and do not restructure the cell: Global Constraint 1 Move 1 names the only six lines that may not be reflowed and this line is not one of them, so a whole-line assertion would turn the suite red on correct text and would grow the no-reflow list from six to seven at the 1079-of-1080 margin, and a blank line in the cell costs a line at that same margin and breaks the list continuation. No plan amendment is needed: the table sits inside a fenced markdown block, which the plan's Body authority note makes reference text, and Task 2's Invariants pin only that the three fallback rows name check (3), check (5) and check (4) with their note lines. Also correct the disposition text, which claims that neither anchor begins a paragraph: only the :397 anchor is imprecise, because the :398 instruction says “the sentence beginning”, which is accurate. Verify by mutation: confirm the updated row needle and the new needle both turn red when the reworded instruction is deleted, restore, and confirm the suite is green again.
- [I1] fixed — the clause-removal instruction now names a bounded run of sentences with an explicit start and an explicit end: the `Clause removed` cell of the `no **Global Constraints:** block` row (skills/multi-doc-review/SKILL.md:676) and both mentions in the prose below the table read “the run of sentences … from `For check (5) report ONE finding` up to, but not including, the sentence beginning `Coverage, ambiguity, feasibility`”, and the closing sentence now says `Stop before the sentence beginning` instead of `Stop at that paragraph`. Both anchor strings are byte-identical to before. The verbatim row needle at tests/reviewer-templates/run-tests.sh:437 was updated in the same commit, and one new `assert_folded_contains` needle in the same section-13 loop pins the reworded bounded-span sentence against `$DOC_SKILL`. No `assert_file_contains` and no `assert_file_has_line` was used, so Global Constraint 1 Move 1's no-reflow list is unchanged; no blank line was added to the lens cell and the cell was not restructured; the prose was reflowed within its own span, so skills/multi-doc-review/SKILL.md stays at 1079 lines against the 1080-line cap of Global Constraint 1. Mutation verification: replacing the reworded row and prose turned exactly the updated row needle and the new needle red (197 passed, 2 failed); after restoring from the pre-mutation copy the file is byte-identical (`diff` prints nothing, `git diff --stat` is empty) and tests/reviewer-templates/run-tests.sh is green at 199 passed, 0 failed, with tests/review-gates (134), tests/writing-plans (15), tests/orchestrating-development (169) and tests/fill-prompt (166) all green as well → 9f4ce7b
- The answer's correction to the disposition text was applied: the `[I1]` line of `## Round 8 verification 6` above now says that only the `:397` anchor begins no paragraph and records that the `:398` instruction, which says “the sentence beginning”, is accurate.

## Round 8 verification 7 — Test & coverage quality — opus
**Reviewers:** M=1, usable 1/1
**Reviewer verdict:** 0 Critical, 0 Important, 4 Minor
### Dispositions
- [M1] carried — the completion-report assertions pin the four labels only, so the `Readiness post: not run (N=0)` value, the `<p> pass(es) — <settled|open after <cap>>` shape and the `Readiness conflicts owed: none` fallback carry no assertion; the reviewer notes the plan's Task 4 Contract verification asks for the four labels only
- [M2] carried — the needle `will be discarded` is three words and pins neither the subject nor the condition of the discard rule, so narrowing that rule leaves the assertion green
- [M3] carried — the needle `as a round is` is four common words asserted against the whole skill file and names neither the readiness pass nor the failure class of the sentence it stands for
- [M4] carried — section 13's needle loop runs against the whole `$DOC_SKILL` although the script already resolves the `### Readiness sequences (plan documents only)` span, so a sentence deleted from that subsection but repeated elsewhere keeps the loop green

Verification re-review run: the effective HEAD was unchanged when this addendum was started, so the accepted fix was verified here instead of by a new invocation entry, and this entry's completion marker was moved to the fix commit 9f4ce7b. No finding of this cycle objects to the fix, and the cycle raised no Critical and no Important finding, so nothing of it is recorded `unresolved: verification cap`.

Note on identifiers: the `[M1]` to `[M4]` of `## Round 8 verification 7` above are NEW items and are none of the same-numbered items of `## Round 8 verification 4`, `5` or `6`. A later answer naming one must be read against this entry.
